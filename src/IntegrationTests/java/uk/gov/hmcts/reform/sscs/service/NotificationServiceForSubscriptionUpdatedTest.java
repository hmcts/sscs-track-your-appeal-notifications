...
public class NotificationServiceForSubscriptionUpdatedTest {
    ...
    private final Subscription emailSubscription = Subscription.builder().tya(APPEAL_NUMBER).email(EMAIL_TEST_1)
        .subscribeEmail(YES).subscribeSms(NO).build();
    private final Subscription smsSubscription = Subscription.builder().tya(APPEAL_NUMBER).mobile(MOBILE_NUMBER_1)
...
//    @Parameters({"appellant"})//, "representative", "appointee"})
    @Parameters(method = "doesNotSendAnyEmailsOrSms")
    public void unsubscribeFromSmsAndEmail_doesNotSendAnyEmailsOrSms(
        Subscription oldAppellantSubscription, Subscription oldAppointeeSubscription, Subscription oldRepSubscription,
        Subscription newAppellantSubscription, Subscription newAppointeeSubscription, Subscription newRepSubscription
    ) {
        doUnsubscribeWithAssertions(
            oldAppellantSubscription,
            oldAppointeeSubscription,
            oldRepSubscription,
            newAppellantSubscription,
            newAppointeeSubscription,
            newRepSubscription
        );
    }

    @SuppressWarnings({"Indentation", "unused"})
    private Object[] doesNotSendAnyEmailsOrSms() {
        return new Object[]{
            // Only Appellant has notifications set
            new Object[]{
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
                null,
                null,
                subscription.toBuilder().subscribeEmail(NO).subscribeSms(NO).build(),
                null,
                null
            },
            new Object[]{
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(NO).build(),
                null,
                null,
                subscription.toBuilder().subscribeEmail(NO).subscribeSms(NO).build(),
                null,
            ...
            new Object[]{
                subscription.toBuilder().subscribeEmail(NO).subscribeSms(YES).build(),
                null,
                null,
                subscription.toBuilder().subscribeEmail(NO).subscribeSms(NO).build(),
                null,
            ...
            // Only Appointee has notifications set
            new Object[]{
                null,
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
                null,
            ...
            // Only Rep has notifications set
            new Object[]{
                null,
                null,
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
            ...
            // Appellant and Rep have notifications set
            new Object[]{
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
                null,
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
            ...
            // Appointee and Rep have notifications set
            new Object[]{
                null,
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
                subscription.toBuilder().subscribeEmail(YES).subscribeSms(YES).build(),
    ...
    public void changeEmail_willSendChangeEmailToOldAndNewEmail(String who) throws NotificationClientException {
        SscsCaseData newSscsCaseData = getSscsCaseData(emailSubscription, who);

        Subscription oldSubscription = emailSubscription.toBuilder().email(EMAIL_TEST_2).build();
        SscsCaseData oldSscsCaseData = getSscsCaseData(oldSubscription, who);
    ...
    private void doUnsubscribeWithAssertions(Subscription newSubscription, String who) {
        doUnsubscribeWithAssertions(subscription, subscription, subscription, newSubscription, null, null);
    }

    private void doUnsubscribeWithAssertions(
        Subscription oldAppellantSubscription, Subscription oldAppointeeSubscription, Subscription oldRepSubscription,
        Subscription newAppellantSubscription, Subscription newAppointeeSubscription, Subscription newRepSubscription
    ) {
        SscsCaseData newSscsCaseData = getSscsCaseData(newAppellantSubscription, newAppointeeSubscription, newRepSubscription);
        SscsCaseData oldSscsCaseData = getSscsCaseData(oldAppellantSubscription, oldAppointeeSubscription, oldRepSubscription);
    ...
    private SscsCaseData getSscsCaseData(Subscription subscription) {
        return getSscsCaseData(subscription, null, null);
    }

    private SscsCaseData getSscsCaseData(Subscription appellantSubscription, Subscription appointeeSubscription, Subscription repSubscription) {
