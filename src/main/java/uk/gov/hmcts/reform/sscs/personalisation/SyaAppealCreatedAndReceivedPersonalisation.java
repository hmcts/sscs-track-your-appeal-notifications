...
public class SyaAppealCreatedAndReceivedPersonalisation extends WithRepresentativePersonalisation {
    ...
    private String buildYourDetails(Appeal appeal) {
        ...
        return NAME + appeal.getAppellant().getName().getFullNameNoTitle() + TWO_NEW_LINES
            + "Date of birth: " + getOptionalField(appeal.getAppellant().getIdentity().getDob(), NOT_PROVIDED)
            + TWO_NEW_LINES + "National Insurance number: " + appeal.getAppellant().getIdentity().getNino()
            + TWO_NEW_LINES + ADDRESS + appeal.getAppellant().getAddress().getFullAddress() + TWO_NEW_LINES
            + EMAIL + getOptionalField(appeal.getAppellant().getContact().getEmail(), NOT_PROVIDED)
            + TWO_NEW_LINES + PHONE + getOptionalField(getPhoneOrMobile(appeal.getAppellant().getContact()), NOT_PROVIDED);
    }
    ...
    private String buildAppointeeDetails(Appointee appointee) {
        ...
        if (StringUtils.equalsIgnoreCase(YES, hasAppointee)) {
            appointeeBuilder.append(TWO_NEW_LINES + NAME).append(appointee.getName().getFullNameNoTitle()).append(TWO_NEW_LINES)
                .append("Date of birth: ").append(appointee.getIdentity().getDob()).append(TWO_NEW_LINES)
                .append(ADDRESS).append(appointee.getAddress().getFullAddress()).append(TWO_NEW_LINES)
                .append(EMAIL).append(getOptionalField(appointee.getContact().getEmail(), NOT_PROVIDED)).append(TWO_NEW_LINES)
                .append(PHONE)
    ...
    private String buildRepresentativeDetails(Representative representative) {
        ...
        if (representative != null && representative.getName() != null && StringUtils.equalsIgnoreCase(YES, hasRepresentative)) {
            representativeBuilder.append(TWO_NEW_LINES + NAME).append(getOptionalField(representative.getName().getFullNameNoTitle(), NOT_PROVIDED)).append(TWO_NEW_LINES)
                .append("Organisation: ").append(getOptionalField(representative.getOrganisation(), NOT_PROVIDED)).append(TWO_NEW_LINES)
                .append(ADDRESS).append(representative.getAddress().getFullAddress()).append(TWO_NEW_LINES)
                .append(EMAIL).append(getOptionalField(representative.getContact().getEmail(), NOT_PROVIDED)).append(TWO_NEW_LINES)
                .append(PHONE)
